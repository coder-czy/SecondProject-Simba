<template>
  <div class="order fixed clearBar">
    <van-nav-bar
      title="我的订单"
      left-text="返回"
      left-arrow
  ...
  methods: {
      ...
      this.axios({
        ...
        .then((result) => {
          ...
          if (result.data.code == 700) {
            ...
            let orderDatas = [];
            let orders = result.data.result.reverse()
            orders.map((v) => {
    ...
    let tokenString = localStorage.getItem('__tk');
        ...
        let data = {
          tokenString,
          appkey: this.appkey,
          count: goods.data[i].count,
          pid: goods.data[i].pid,
          rule:goods.data[i].rule
        };
          ...
          this.axios({
            ...
            if (result.data.code == 700) {
             ...
             return this.$router.push({name: 'Login'});
              ...
              // this.shopbagData.push(result.data)
              // this.sids.push(result.data.sid)
              // 
              sids.push(result.data.sid)
              if(i==goods.data.length-1){
                resolve(sids)
              }
        ...
        // resolve()
          ...
          // 
        //   let sid =this.res.join("-")
        
        // 
      this.$router.push({ name: "Pay", query: { sids: res.join("-") } });
        })
